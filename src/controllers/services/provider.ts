...
export default async function providerStatus(
  ...
  try {
    ...
    const info = initializeInfo(network)
    const fileInfo = (await providerRequest('fileinfo', fileInfoBody))[0]
    const endpoint = `https://v4.provider.oceanprotocol.com/api/services/initialize?documentId=did:op:${info.did}&serviceId=${info.serviceId}&fileIndex=0&consumerAddress=0x0000000000000000000000000000000000000000`
