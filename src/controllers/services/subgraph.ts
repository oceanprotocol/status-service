...
export default async function subgraphStatus(network: Network) {
  ...
  console.log(process.env.BLOCK_TOLERANCE)
  console.log(Number(process.env.BLOCK_TOLERANCE))
  const blockCheck = data._meta.number + Number(process.env.BLOCK_TOLERANCE)
